...
public class PSOSearch extends ASSearch 
	...
	throws Exception {
		...
		if (!getLogFile().isDirectory()) {
			...
			message.append("\nBest fitness found: " + m_totalBest.getObjective() 
					+ ", with " + countFeatures(m_totalBest.getCluster()) + " features selected.\n");
			message.append("Best iteration found: " + bestI + "\n");
			message.append("Selected features are: " + printPopMember(m_totalBest.getCluster()) + "\n");
		...
		return attributeList(m_totalBest.getCluster());
	...
	throws Exception {
		...
		for (i=0;i<m_popSize;i++) {
			...
			if (m_lookupTable.containsKey(m_population[i].getCluster()) == false) {
				merit = ASEvaluator.evaluateSubset(m_population[i].getCluster());
				m_population[i].setObjective(merit);
				auxPSOBitSet = (PSOBitSet)m_population[i].clone();
				auxBitSet = (BitSet)m_population[i].getCluster().clone();
				m_lookupTable.put(auxBitSet, auxPSOBitSet);
			} else {
				auxPSOBitSet = (PSOBitSet)m_lookupTable.get(m_population[i].getCluster());
				m_population[i].setObjective(auxPSOBitSet.getObjective());				
			} // if lookupTable
			// update m_bestPopulation
			updateBestPopulation(i, m_population[i].getObjective(), m_population[i].getCluster());
		} // for popSize
	} // evaluatePopulation
	...
	throws Exception {
		...
		if (merit > m_bestPopulation[index].getObjective()) {
			...
			if (countFeatures(chromosome) < countFeatures(m_bestPopulation[index].getCluster())) {
				m_bestPopulation[index] = (PSOBitSet)m_population[index].clone();			
			}
		}
	...
	private void checkBest() throws Exception {
		...
		int currentBestCount = countFeatures(currentBest.getCluster()); 
		...
		if (m_maxFitness - m_minFitness > 0) {
			...
			for (i=1;i<m_popSize;i++) {
				if (m_population[i].getObjective() > currentBestFitness) {
					...
					currentBestCount = countFeatures(currentBest.getCluster());
				} else if (Utils.eq(m_population[i].getObjective(), currentBestFitness)) {
					// see if it contains fewer features
					count = countFeatures(m_population[i].getCluster());
...
//			temp.append(Utils.doubleToString(Math.abs(m_population[i].getObjective()),8,5)
			temp.append(Utils.doubleToString(m_population[i].getObjective(),8,5)
					...
					+"\t");
			temp.append(printPopMember(m_population[i].getCluster())+"\n");
		}
	...
	private void generation() throws Exception {
		...
		for (i = 0; i < m_popSize; i++) {
			...
			m_population[i].threePBMCX(m_random, 
					...
					m_partialBest.getCluster(),m_bestPopulation[i].getCluster());
